...
class RHAdmin_V01():
    ...
    def createUsers(self, service, users):
        failed = []
        for aUser in users:
            if aUser['positionUpdate'] == 'no':
                pass
            newData = {'name':aUser['name'],'primaryEmail':aUser['primaryEmail'],'password':aUser['password']}
            try:
                service.users().insert(newData).execute()
                self.log('info', 'Successfully Created User ' + aUser['name'] + ' ' + ' ' + aUser['email'])
            except:
                self.log('debug', 'Failed to Create User ' + aUser['name'] + ' ' + aUser['email'])
                failed.append(aUser)
        return failed

    def updateUsers(self, service, users):
        failed = []
        for aUser in users:
            key = ''
            if aUser['oldEmail'] == 'no':
                key = aUser['primaryEmail']
            else:
                key = aUser['oldEmail']
            newData = {'name':aUser['name'],'primaryEmail':aUser['primaryEmail'],'password':aUser['password']}
            try:
                service.users().insert(userKey=key, body=newData).execute()
                self.log('info', 'Successfully Created User ' + aUser['name'] + ' ' + ' ' + aUser['email'])
            except:
                self.log('debug', 'Failed to Create User ' + aUser['name'] + ' ' + aUser['email'])
                failed.append(aUser)
